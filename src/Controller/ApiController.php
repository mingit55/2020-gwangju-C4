...
class ApiController {
    ...
    function getFestivalList(){
        json_response([
            "festivals" => array_map(function($festival){
                ...
                return $festival;
            }, DB::fetchAll("SELECT * FROM festivals ORDER BY id ASC")),
        ]);
    }   
    ...
    // 데이터 공개 API
    function getFestivalListByDate(){
        // 유효성 검사
        $searchType = isset($_GET['searchType']) && array_search($_GET['searchType'], ['M', 'Y']) !== false ? $_GET['searchType'] : null;
        $year = isset($_GET['year']) && strtotime($_GET['year']."-01-01") !== false ? $_GET['year'] : null;
        if(!$searchType || !$year) exit;
        if($searchType == "Y") $month = "1";
        else {
            $month = isset($_GET['month']) && strtotime("2020-".$_GET['month']."-01") !== false ? $_GET['month'] : null;
            if(!$month) exit;
        }
        

        // 데이터 검색
        $t_start_date = strtotime("$year-$month-1");
        $t_end_date = $searchType == "M" ? 
                        strtotime("-1 Day", strtotime("+1 Month", $t_start_date)) 
                        : strtotime("$year-12-31");
        $start_date = date("Y-m-d", $t_start_date);
        $end_date = date("Y-m-d", $t_end_date);

        $items = array_map(function($item){
                $item->images = array_map(function($image){
                    return $image->local_name;
                }, DB::fetchAll("SELECT * FROM images WHERE fid = ?", [$item->sn]));
                return $item;
            }, DB::fetchAll("SELECT 
                                id sn,
                                no no,
